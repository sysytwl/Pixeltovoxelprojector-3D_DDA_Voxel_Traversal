                ...
                if (voxel_grid_provided)
                {
                    ...
                    if (ray_aabb_intersection(ray_origin, ray_direction, box_min, box_max, t_entry, t_exit))
                    {
                        ...
                        if (t_entry <= t_exit)
                        {
                            ...
                            double px = ray_origin[0] + t_entry * ray_direction[0];
                            double py = ray_origin[1] + t_entry * ray_direction[1];
                            double pz = ray_origin[2] + t_entry * ray_direction[2];

                            // Get voxel indices for starting point
                            ...
                            if (x < 0) continue; // Outside grid

                            // Compute voxel size
                            double voxel_size_x = (x_max - x_min) / voxel_grid_size[0];
                            double voxel_size_y = (y_max - y_min) / voxel_grid_size[1];
                            double voxel_size_z = (z_max - z_min) / voxel_grid_size[2];

                            // Step direction
                            int step_x = (ray_direction[0] > 0) ? 1 : -1;
                            int step_y = (ray_direction[1] > 0) ? 1 : -1;
                            int step_z = (ray_direction[2] > 0) ? 1 : -1;

                            // Next voxel boundary
                            double next_voxel_boundary_x = x_min + (x + (step_x > 0 ? 1 : 0)) * voxel_size_x;
                            double next_voxel_boundary_y = y_min + (y + (step_y > 0 ? 1 : 0)) * voxel_size_y;
                            double next_voxel_boundary_z = z_min + (z + (step_z > 0 ? 1 : 0)) * voxel_size_z;

                            // tMax: distance along ray to next voxel boundary
                            double tMaxX = (ray_direction[0] != 0) ? (next_voxel_boundary_x - ray_origin[0]) / ray_direction[0] : std::numeric_limits<double>::infinity();
                            double tMaxY = (ray_direction[1] != 0) ? (next_voxel_boundary_y - ray_origin[1]) / ray_direction[1] : std::numeric_limits<double>::infinity();
                            double tMaxZ = (ray_direction[2] != 0) ? (next_voxel_boundary_z - ray_origin[2]) / ray_direction[2] : std::numeric_limits<double>::infinity();

                            // tDelta: distance along ray to cross one voxel
                            double tDeltaX = (ray_direction[0] != 0) ? voxel_size_x / std::abs(ray_direction[0]) : std::numeric_limits<double>::infinity();
                            double tDeltaY = (ray_direction[1] != 0) ? voxel_size_y / std::abs(ray_direction[1]) : std::numeric_limits<double>::infinity();
                            double tDeltaZ = (ray_direction[2] != 0) ? voxel_size_z / std::abs(ray_direction[2]) : std::numeric_limits<double>::infinity();

                            double t = t_entry;
                            while (x >= 0 && x < voxel_grid_size[0] &&
                                   y >= 0 && y < voxel_grid_size[1] &&
                                   z >= 0 && z < voxel_grid_size[2] &&
                                   t <= t_exit)
                            {
                                #pragma omp atomic
                                voxel_grid_mutable(x, y, z) += brightness;
